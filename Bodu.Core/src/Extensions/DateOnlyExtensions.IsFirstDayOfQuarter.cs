...
namespace Bodu.Extensions
{
	public static partial class DateOnlyExtensions
	{
		...
		/// <returns><see langword="true" /> if the <paramref name="dateTime" /> is the first day of its quarter; otherwise, <see langword="false" />.</returns>
		/// <remarks>
		/// This method evaluates whether the date component (ignoring the time of day) of the <paramref name="dateTime" /> corresponds to
		/// the first day of the quarter based on the standard calendar year definition (January–March as Q1, etc.). The comparison is
		/// performed using the <see cref="DateOnly" /> property to normalize the time to midnight before evaluating the boundary.
		/// </remarks>
		public static bool IsFirstDayOfQuarter(this DateOnly dateTime)
		{
			var (year, quarter) = GetQuarterAndYearFromDate(CalendarQuarterDefinition.JanuaryDecember, referenceDate: dateTime);
			return dateTime.DayNumber == ComputeQuarterStartDayNumber(year, quarter, GetQuarterDefinition(CalendarQuarterDefinition.JanuaryDecember));
		}
		...
		/// <returns>
		/// <see langword="true" /> if the <paramref name="dateTime" /> is the first day of its quarter based on the specified
		...
		/// <remarks>
		/// This method evaluates whether the date component (ignoring the time of day) of the <paramref name="dateTime" /> corresponds to
		/// the first day of the quarter based on the provided <paramref name="definition" />. The comparison is performed using the
		/// <see cref="DateOnly" /> property to normalize the time to midnight before evaluating the boundary.
		/// </remarks>
		public static bool IsFirstDayOfQuarter(this DateOnly dateTime, CalendarQuarterDefinition definition)
		{
			...
			return dateTime.DayNumber == ComputeQuarterStartDayNumber(year, quarter, GetQuarterDefinition(definition));
		}
		...
		/// </param>
		/// <returns>
		/// <see langword="true" /> if the <paramref name="dateTime" /> is the first day of its quarter according to the rules of the
		...
		/// <remarks>
		/// This method evaluates whether the date component (ignoring the time of day) of the <paramref name="dateTime" /> corresponds to
		/// the first day of the quarter, using the logic defined by the specified <paramref name="provider" />. The comparison is performed
		/// using the <see cref="DateOnly" /> property to normalize the time to midnight before evaluating the boundary.
		/// </remarks>
		public static bool IsFirstDayOfQuarter(this DateOnly dateTime, IQuarterDefinitionProvider provider)
		{
			ThrowHelper.ThrowIfNull(provider);
			return dateTime.DayNumber == provider.GetStartDate(dateTime).DayNumber;
		}
	}
}