...
namespace Bodu.Extensions
{
	public static partial class DateOnlyExtensions
	{
		...
		/// <summary>
		/// Returns the 1-based week number of the month for the specified <see cref="DateOnly" />, using the calendar week rule and first
		...
		public static int WeekOfMonth(this DateOnly date, CalendarWeekRule weekRule, DayOfWeek weekStart)
		{
			...
			var weekOfYear = calendar.GetWeekOfYear(date.ToDateTime(TimeOnly.MinValue), weekRule, weekStart);
			var firstOfMonth = DateOnly.FromDayNumber(DateOnlyExtensions.GetDayNumber(date.Year, date.Month, 1));
			var firstWeek = calendar.GetWeekOfYear(firstOfMonth.ToDateTime(TimeOnly.MinValue), weekRule, weekStart);
