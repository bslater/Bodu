	...
	public partial class CrcTests
	{
		...
		public override IEnumerable<HashAlgorithmVariant> GetVariants() => new[]
		{
			new HashAlgorithmVariant
			{
				...
				ExpectedHash_ForSimpleTextAsciiBytes = "39A34F41",
				ExpectedHash_ForByteSequence0To255 = "738C0529",
		...
		/// </summary>
		/// <remarks>
		/// This method creates a new instance of the <see cref="CrcTests" /> class and yields test case data for each
		/// <see cref="HashAlgorithmVariant" /> defined in <see cref="GetVariants" />. It is intended to be used as a data source for
		/// <c>[DataTestMethod]</c> tests that validate different configurations or behaviors of the <see cref="Crc" /> hash algorithm.
		/// </remarks>
		/// <returns>
		...
		{
			var instance = new CrcTests();
			foreach (var variant in instance.GetVariants())
				yield return new object[] { variant };
		}

		/// <summary> Verifies that <see cref="Crc"> produces expected hash for an empty byte array. </summary>
		[DataTestMethod]
		...
		/// <summary> Verifies <see cref="Crc"> hash for simple ASCII text. </summary>
		[DataTestMethod]
		[DynamicData(nameof(GetTestVariants), DynamicDataSourceType.Method)]
		...
		/// <summary> Verifies <see cref="Crc"> hash for byte sequence 0-255. </summary>
		[DataTestMethod]
		[DynamicData(nameof(GetTestVariants), DynamicDataSourceType.Method)]
		...
		/// <summary> Verifies that hashing progressively longer prefixes using the <see cref="Crc"> hash produces expected results using. </summary>
		[DataTestMethod]
		[DynamicData(nameof(GetTestVariants), DynamicDataSourceType.Method)]
