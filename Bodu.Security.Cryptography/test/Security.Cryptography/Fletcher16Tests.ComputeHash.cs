...
namespace Bodu.Security.Cryptography
{
	public partial class Fletcher16Tests
	{
		...
		public override IEnumerable<HashAlgorithmVariant> GetVariants() => new[]
		{
			...
			{
				Name = "Fletcher16",
				Factory = () => new Fletcher16(),
				ExpectedHash_ForEmptyByteArray = "0000",
				ExpectedHash_ForSimpleTextAsciiBytes = "FEE8",
				ExpectedHash_ForByteSequence0To255 = "5500",
				...
				{
					new HashAlgorithmTestCase{ InputHex="abcde", ExpectedHex="C8F0" },
					new HashAlgorithmTestCase{ InputHex="abcdef", ExpectedHex="2057" },
					new HashAlgorithmTestCase{ InputHex="abcdefgh", ExpectedHex="0627" },
				}
			},
		};
		...
		/// </summary>
		/// <remarks>
		/// This method creates a new instance of the <see cref="Fletcher16Tests" /> class and yields test case data for each
		/// <see cref="HashAlgorithmVariant" /> defined in <see cref="GetVariants" />. It is intended to be used as a data source for
		/// <c>[DataTestMethod]</c> tests that validate different configurations or behaviors of the <see cref="Fletcher16" /> hash algorithm.
		/// </remarks>
		/// <returns>
		...
		{
			var instance = new Fletcher16Tests();
			foreach (var variant in instance.GetVariants())
				yield return new object[] { variant };
		}

		/// <summary> Verifies that <see cref="Fletcher16"> produces expected hash for an empty byte array. </summary>
		[DataTestMethod]
		...
		/// <summary> Verifies <see cref="Fletcher16"> hash for simple ASCII text. </summary>
		[DataTestMethod]
		[DynamicData(nameof(GetTestVariants), DynamicDataSourceType.Method)]
		...
		/// <summary> Verifies <see cref="Fletcher16"> hash for byte sequence 0-255. </summary>
		[DataTestMethod]
		[DynamicData(nameof(GetTestVariants), DynamicDataSourceType.Method)]
