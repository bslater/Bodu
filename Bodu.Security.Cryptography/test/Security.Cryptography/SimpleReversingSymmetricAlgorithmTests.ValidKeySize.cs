...
namespace Bodu.Security.Cryptography
{
	...
	{
		/// <summary>
		/// Verifies that <see cref="TweakableSymmetricAlgorithm.ValidKeySize(int)" /> returns <c>true</c> for values that are expected to
		...
		public void ValidTweakSize_WhenLengthIsValid_ShouldReturnTrue(int length)
		{
			...
			bool result = algorithm.ValidKeySize(length);
			Assert.IsTrue(result);
		}

		// <summary>
		/// Verifies that <see cref="TweakableSymmetricAlgorithm.ValidKeySize(int)"/> returns <c>false</c> for values that are not
